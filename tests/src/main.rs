...
use shared_global::{SENSOR_REGISTRY, SensorType, UnifiedReading};
use std::sync::Arc;
use std::thread;
...
fn main() {
    ...
    let buffer = Arc::new(BufferManager::<UnifiedReading>::new(50000));
    // 1. 创建、启动并注册传感器
    for i in 0..300 {
        let sensor_type = rng.gen_range(0..3);
        // let rate = rng.gen_range(900..1001);
        let rate = 1000;
    ...
    // 5. 主线程监控
    println!("测试 BufferManager");

    // 运行一段时间后退出（避免无限运行）
    let monitor_duration = Duration::from_secs(7);
