...
/// 多线程版的通用缓冲区管理器，底层用 VecDeque 做环形队列。
...
pub struct BufferManager<T> {
    ...
    start_time: Instant,
    sensor_rates: Mutex<HashMap<String, u32>>,
    urgent_queue: Mutex<Vec<String>>, // 紧急传感器ID列表（剩余时间 < 10ms）
    normal_queue: Mutex<Vec<String>>,
}
...
impl BufferManager<UnifiedReading> {
    ...
    pub fn new(capacity: usize) -> Self {
        ...
        Self {
            ...
            urgent_queue: Mutex::new(Vec::new()),
            normal_queue: Mutex::new(Vec::new()),
        }
    }
                ...
                match parts[0] {
                    ...
                    "AVAIL" => {
                        ...
                        if parts.len() == 2 {
                            ...
                            let rate_opt = {
                                let rates = manager.sensor_rates.lock().unwrap();
                                rates.get(&sensor_id).cloned()
                            };
                            if let Some(rate) = rate_opt {
                                let emergency = (127 - avail) as f64 / rate as f64;
                                let mut urgent = manager.urgent_queue.lock().unwrap();
                                let mut normal = manager.normal_queue.lock().unwrap();

                                if emergency < 0.01 {
                                    // 加入紧急队列
                                    if !urgent.contains(&sensor_id) {
                                        urgent.push(sensor_id.clone());
                                    }
                                    // 从普通队列移除
                                    if let Some(pos) = normal.iter().position(|x| x == &sensor_id) {
                                        normal.remove(pos);
                                    }
                                } else {
                                    // 加入普通队列
                                    if !normal.contains(&sensor_id) {
                                        normal.push(sensor_id.clone());
                                    }
                                    // 从紧急队列移除
                                    if let Some(pos) = urgent.iter().position(|x| x == &sensor_id) {
                                        urgent.remove(pos);
                                    }
                                }
                            }
